...
class TorController {
  constructor(options = {}) {
    const localBinTor = path.resolve(__dirname, 'bin/tor/tor.exe');
    const legacyTor = path.resolve(__dirname, '../tor-ip-changer/tor/tor.exe');
    this.torExe = options.torExe || (fs.existsSync(localBinTor) ? localBinTor : (fs.existsSync(legacyTor) ? legacyTor : 'tor'));

    this.dataDir = options.dataDir || path.resolve(__dirname, 'tor_data');

    const localGeoip = path.resolve(__dirname, 'bin/tor/geoip');
    const legacyGeoip = path.resolve(__dirname, '../tor-ip-changer/tor/geoip');
    this.geoipFile = options.geoipFile || (fs.existsSync(localGeoip) ? localGeoip : legacyGeoip);

    const localGeoip6 = path.resolve(__dirname, 'bin/tor/geoip6');
    const legacyGeoip6 = path.resolve(__dirname, '../tor-ip-changer/tor/geoip6');
    this.geoip6File = options.geoip6File || (fs.existsSync(localGeoip6) ? localGeoip6 : legacyGeoip6);
